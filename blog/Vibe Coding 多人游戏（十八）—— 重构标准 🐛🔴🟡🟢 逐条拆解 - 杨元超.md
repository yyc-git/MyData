...
- E2E 场景验证通过（Playwright 自动化浏览器测试）
...
第三轮是 AI 自动执行的代码审核，不是人肉检查。代码审核用独立会话加载完整审查标准，逐项对照。当 Maker 和 Checker 用同一模型时，Checker 默认信任 Maker 的实现，不会深挖——所以 GTS-Play 中审核会话使用更强的模型，Maker 使用快速模型，形成独立审计。

有一次 AI 重构了 `Server.ts` 的 `_handlePlayerDisconnect` 函数，顺手把主机转移的逻辑也改了——加了一个广播副作用混入纯函数。代码审核里发现了，标记为 🔴：广播逻辑从纯数据转换中分离出来，由调用方负责。
...
以下是从 workflow-rules.md 提取的完整审查标准，每次代码审核逐项检查。

### 🐛 Bug 检查（必须修）

- **空指针/undefined 访问** — 可选链（`?.`）遗漏、条件分支覆盖不全
- **边界条件遗漏** — 数组越界、数值边界、空集合循环
- **异步竞态条件** — Promise 时序问题、多处同时修改同一状态
- **状态更新逻辑错误** — 修改了 state 但未同步到 UI，或误改了影响其他模块的状态
- **类型安全** — `as any` 硬转、函数签名不匹配、类型断言风险
...
- **改动真正生效了吗？** — AI 可能改了一堆代码但实际没执行到。检查关键策略是否真正被调用/生效
- **End 逻辑重置检查** — 游戏退出/销毁/dispose/stop 时所有跨轮状态是否清干净
...
**架构与结构**
- 是否符合开闭原则：增功能时扩展现有架构，而非改现有单机代码路径
- 优先事件驱动而非 if-else 条件分支/轮询
- 不在中间 require/import 文件，统一在最开头引入
- 文件 >500 行 → 按 export 拆文件
- 函数 >100 行 → 提子函数
- 重复代码 ≥3 处 → 提公共函数
- 避免硬编码：magic number、inline string 抽为常量/枚举/配置对象
- **警惕不必要的抽象/过度设计** — 一个使用场景不需要接口/工厂/策略模式全套
- **代码是否太长/复杂？** — AI 常把简单事情复杂化，两行判断拆成三个函数加一个类
- **有没有产生重复代码？** — 检查是否和现有 util 重复
- **有没有破坏已有的代码风格？** — 同项目多种写法混搭
- **改动范围是否合理？** — 确认每个改动都是必要且合理的

**测试质量**
- BDD 测试必须测试实际代码，不能有假测试（mock-only、无断言的空测试）
- 集成测试覆盖尽可能多的真实流程，优先走完整管线入口
- 测试 mock 过多 → 重构生产函数
...
- 函数隐式调 readState/writeState：应通过参数传入

**安全与可靠**
- 使用项目自定义 Three.js class，禁止直接 `new THREE.Sprite`
...
- 导出函数缺 JSDoc → 补注释
- 空 catch 块静默吞错
- 未清理的 event listener/timer/全局对象
- 异步调用无超时/错误处理

### 代码审核回复规则

> 审核不是一次性的，而是一问一答的对话。AI 查出问题后，Maker 必须逐一回应。
> 没有明确回复 = 默认要修。只有写了「不处理」「跳过」「忽略」才跳过。
