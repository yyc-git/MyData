...
**1. 入口检查**：每次收到新消息先检查后台任务是否完成。有一次我连续发了两个消息："修 A bug"和"修 B bug"，OpenClaw 两条都开始处理，结果两个 OpenCode 实例同时修改同一个文件，冲突了。后来加了规则：每次收到新消息先检查 `process list`，如果有子任务在跑，先汇报完成再接新任务。

**2. 先汇报再继续**：OpenCode 出结果后，必须出声总结再下一步。有一次 OpenCode 默默修完了 bug 然后等下一个指令，中间隔了 5 分钟我才发现它已经完成了。规则改成：OpenCode 出结果后 OpenClaw 必须立刻汇报摘要，然后才继续下一步。

**3. 等确认发通知**：需要我决策时，发桌面通知 + 飞书消息双重通知。为什么用双通道？因为我有一次在飞书上等 OpenClaw 的结果，但它悄悄跑去部署了，我完全不知道——因为部署指令是在飞书对话里发的，但我在看其他地方。加了桌面通知后，`msg * "请确认部署方案"` 这种弹窗确保我不会错过来自 AI 的决策请求。

**4. 双通道通知**：重要事件走两个通道：
   - 桌面弹窗：`msg * "<消息>"`，确保即使我没看聊天工具也能被提醒
   - 聊天工具补充：通过 cron announce 的 feishu 或 clickclack 通道

**5. 保存节奏**：每完成一个迭代，自动触发 `gts-submit-save`——git commit + 记忆保存。不 push（push 是手动触发的，因为线上部署需要谨慎）。记忆保存包括：daily log、决策记录（ADR）、修改文件清单。这些记忆在下次搜索时就会用到。
...
**但这里面有一个隐形成本——"确认等待"。** 如果我在开会或者睡觉，流程就卡住了。所以对于小型改动（比如改个配置属性、修复一个不影响流程的日志级别），OpenClaw 有一个快捷模式：无脑修，修完通知我。不需要我确认 Brief，不需要我确认方案。
