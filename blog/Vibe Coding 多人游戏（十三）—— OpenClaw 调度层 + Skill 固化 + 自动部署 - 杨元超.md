...
我需要的只是指示 OpenClaw 去做这些步骤。如果这些步骤每一步都要我手动管理，那分层省下的时间又浪费到手动管理流程上了。
...
以下是全部 16 个 Skill 的完整 `SKILL.md` 定义：
...
> 双窗口 + CDP Profiler 录制，用户操控，自动分析摘要 + 短通知。
> 基于积木系统：`e2e-runner.cjs scenarios/perf-scene3.json`
...
运行命令：
```
node D:\Github\GTS-Play\packages\frontend\test\e2e\e2e-runner.cjs scenarios/perf-scene3.json
```
...
- 工作目录：`D:\Github\GTS-Play\packages\frontend\test\e2e`
- 以 background 模式运行
- 启动后告知兄弟：「✅ 性能录制已就绪，双窗口并排，点要录制的窗口上的「▶ 录制此窗口」，操作角色，点停止结束」

### Step 2：等待录制完成

- 不 poll，等 scenario 自然退出
- 用户自己操控：点开始 → 操作角色 → 点停止
...
-- API 耗时 --
  AllCommands: 平均 50ms | 最大 104ms | 330 次
...
- 时间：<日期>
- 场景：perf-scene3
- 运行方式：node e2e-runner.cjs scenarios/perf-scene3.json
- FPS 平均/最低：58/11
...
1. 跑固定 scenarios `perf-scene3.json`，不动态生成
2. URL 使用 `?user=t1` / `?user=t2` 免登录
3. 性能数据（CDP Profiler 原始数据 + FPS 记录）保存到 `test/e2e/perf-data/`（已 gitignore），不删除
4. 完整日志保存到 `test/e2e/__logs__/`（已 gitignore），不删除
5. 不 poll，等 scenario 自然退出
6. 分析结果在会话中给出，通知只发短提醒
7. 和 `gts-e2e-test`（手动双窗口）、`gts-e2e-auto`（自动验收）互不触发
8. **E2E 测试中发现错误、兄弟喊我修复/分析时** → 必须调度 OpenCode 处理（`opencode run`），**brief 开头自动注入 `笔记/项目文档/project-context.md`**，把完整上下文（日志/截图/错误信息）传给 OpenCode。**我自己不分析日志、不自己写修复代码**。
...
description: "兄弟说「记忆」「保存记忆」「保存笔记」时触发。保存daily log+commit，保存项目笔记+commit，不push，不动.last-save。"
---

# gts-save-memory — 记忆与笔记保存

> 兄弟说「记忆」「保存记忆」「保存笔记」时触发。
> 记忆和笔记都做 git commit。不 push。不动 `.last-save`（由保存skill管理）。

## 功能

### 保存记忆
- 将当前对话的关键要点写入 `workspace/memory/YYYY-MM-DD-log.md`
- 格式：日期时间 + 对话摘要 + 关键决策表 + 当前状态
- 覆盖同一日的 daily log（不要追加重复）
- 记忆写完后 → `git add workspace/memory/` → `git commit -m "save memory YYYY-MM-DD"`

### 保存笔记
- 将本次对话中新增的项目知识写入 `笔记/` 对应目录
- 按内容类型归入：项目文档/、决策记录/、方案/、讨论记录/、代码笔记/
- 只写新的知识，不重写已有内容
- 笔记写完后 → `git add 笔记/` → `git commit -m "update notes YYYY-MM-DD"`

### 不做什么
- ❌ 不动 `.last-save`（由保存skill gts-save-flow 管理）
- ❌ 不 push
- ❌ 不动项目代码（只动 memory/ + 笔记/）

## 规则

- 如果只说「记忆」→ 只保存记忆 + commit
- 如果只说「保存记忆」→ 只保存记忆 + commit
- 如果只说「保存笔记」→ 只保存笔记 + commit
- 如果同时提到 → 两个都 commit
- 结束后直接告知兄弟「已保存」
...
---
name: "gts-analysis"
description: "分析：调度OpenCode Pro分析需求/方案，我转达结果给兄弟。"
---

# gts-analysis — OpenCode 分析调度

> 兄弟说「分析」或遇到复杂需求自动触发。
> **我不自己分析**，直接调度 OpenCode Pro。
> 具体命令模板见 `skills/opencode-schedule/SKILL.md`（Pro + max）。

### Step 2：转达结果

- 把 OpenCode 的分析结果/方案给兄弟看
- ⏸️ 等兄弟确认后再决定下一步

### Step 3：更新笔记

- 分析结果写入 `笔记/方案/` 或 `笔记/讨论记录/`
- 📣 发通知（桌面消息 + 飞书推送）
...
一次 `memory_search` 搞定，不要分多次搜：

```
memory_search(query: "最近工作 改动 修复 重构 保存", maxResults: 5, minScore: 0.3)
```

同时读 `笔记/决策记录/.last-save` 获取最近保存 SHA。
...
只看最近 5 条纯对话内容，**不带工具调用/exec 输出/文件内容**。足够了解兄弟在做什么。
...
- 搜不到就说搜不到，不编造
- `memory_search` 仅一次，不高频重试
- `sessions_history` 不带工具内容
- 输出固定短格式，不啰嗦
...
2. 如果有正在跑的 OpenCode → `process(action=kill, sessionId=<id>)` 杀掉
3. 如果有正在跑的服务/脚本 → `process(action=kill, sessionId=<id>)` 杀掉
4. 不要自己主动重启任何东西
