...
这个决定在当时是自然的：多人联网的逻辑太复杂了，我需要先自己搞清楚每一行是干什么的，后续才能让 AI 接手。不了解问题就去用 AI，跟"不会游泳就跳进深水区"一样——AI 不仅解决不了问题，反而会让问题更隐蔽。
...
不过我没有直接从 basic1 跳到最终版。下一阶段是**让 AI 参考 basic1 的代码，改出一个状态同步版本**——这就是 new_basic1（后来迭代为 new_basic2）。

basic1 的代码在后续重构中基本废弃，但这两周的手写经历，踩的每一个坑，都直接决定了后面的技术选型。没有这两周的帧同步痛苦，后面引入 AI 时我根本不知道怎么描述需求——知道「不要什么」和知道「要什么」同等重要。
