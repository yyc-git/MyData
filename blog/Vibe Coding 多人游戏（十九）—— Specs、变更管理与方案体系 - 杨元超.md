...
我经历过最惨的一次：需求是重置 `isEnterGame` 标志位——一个很简单的问题。AI 理解后，花了一整天重构了退出房间的整个流程：加了重连身份校验、改了玩家队列逻辑、还给挂机玩家加了自动踢出。等代码写完，我发现 80% 的改动跟标志位重置毫无关系。AI 不是不理解问题，而是它"顺便"想改善一下退出体验。那天烧掉了近百元 token，至少 80% 是白费的。
...
所以 `gts-save-flow` skill（结束当天开发时调用一次，不是每次变更都跑）的流程里专门有一道关卡：**规格同步**。审核 → BDD → 编译 → 规格 → 笔记 → 记忆 → 提交。规格位于笔记之前，就是要确保代码和文档保持同步。
